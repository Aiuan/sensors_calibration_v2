...
class VelodyneDecoder(object):
    def __init__(self, config_path, pcap_path, output_path, n_skip_packages=0, frame_cut_degree=180):
        print('=' * 100)
        ...
        self.n_skip_packages = n_skip_packages
        if self.n_skip_packages > 0:
            for i in range(self.n_skip_packages):
                self.idx_packet, (ts, pkg) = next(self.reader)
                print('idx_packet={}, skip'.format(self.idx_packet))
    ...
    def select_generate_frame(self, timestamps, day, pcd_file_type='pcd'):
        # name the file according to the pts' unix_timestamp at 0 degree
        timestamp = self.pts_frame['unix_timestamp'].iloc[np.abs(self.pts_frame['azi'].values).argmin()]

        if np.abs(timestamp - timestamps).min() > 0.1 / 2:
            timestamp_idx_matched = None
            log_YELLOW('    Skip generate {:.6f}.{}'.format(timestamp, pcd_file_type))
        else:
            timestamp_idx_matched = np.abs(timestamp - timestamps).argmin()

            pcd_filename = '{}_{}.{}'.format(day, timestamp_idx_matched+1, pcd_file_type)
            pcd_path = os.path.join(self.output_path, pcd_filename)
