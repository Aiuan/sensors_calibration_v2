...
raw_data_folders = [
    fullfile(raw_data_root, '20221226_calibration\cameras_radars_calibration\cameras')
    ];
...
for k = 1:size(raw_data_folders, 1)
    ...
    for i = 1:length(filenames)
        if strcmp(filenames(i).name, '.') || strcmp(filenames(i).name, '..') || strcmp(filenames(i).name, 'readme.txt')
            continue;
        end
        ...
        if ~ismember(data(cnt_data+1).timestamp, timestamps)
            ...
            if length(timestamps) == 1
                % 2nd forget take photo
                timestamps{length(timestamps)+1} = '0';
            end
        end
