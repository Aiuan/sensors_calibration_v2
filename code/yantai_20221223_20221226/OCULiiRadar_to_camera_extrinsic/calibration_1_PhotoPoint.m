...
tmp = cell(length(image_list_name)+1, 1);
for i = 1:length(image_list_name)
    if i<2
        tmp{i} = image_list_name{i};
        tmp{i+1} = '';        
    else
        tmp{i+1} = image_list_name{i};
    end
end
image_list_name = tmp;

points_pixel = zeros(size(image_list_name, 1), 2);
figure(1);
for i=1:length(image_list_name)
    disp('=====================================================');
    if i == 2
        disp('frameIdx=2 lost');
        continue;        
    end
    
    image_path = fullfile(image_folder, image_list_name{i});
    disp(image_list_name{i});
