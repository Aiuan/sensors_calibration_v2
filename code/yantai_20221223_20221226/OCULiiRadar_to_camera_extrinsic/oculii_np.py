...
class HandshakePacket(object):
  def __init__(self, idx_packet, timestamp_packet_receive, bytes_stream):
    ...
    self.timestamp_trigger = self.timestamp_packet_receive
    ...
    self.is_ptp_sync = (np.abs(delay) <= 10)
...
class BodyPacket_divided(object):
  ...
  @staticmethod
  def is_BodyPacket_divided(bytes_stream):
    if len(bytes_stream) <= 42 + 256:
      eth = dpkt.ethernet.Ethernet(bytes_stream)
      src = socket.inet_ntoa(eth.data.src)
      dst = socket.inet_ntoa(eth.data.dst)
      sport = eth.data.data.sport
      dport = eth.data.data.dport
      if src == '192.168.2.14' and dst == '192.168.2.65' and sport == 9909 and dport == 9914:
        return True
      else:
        return False
    else:
...
class OCULiiDecoderNetworkPackets(object):
    ...
    def next_frame_packets(self):
      ...
      while cnt_length < handshake_pkg.frame_data_length:
        idx_packet, ts, pkg = self.next_udp_packet()
        if BodyPacket_divided.is_BodyPacket_divided(pkg):
          ...
          print('[{:.6f}] idx_packet={}, not bodypacket_divided, skip'.format(ts, idx_packet))
      ...
      while True:
        try:
          ...
          if hasattr(eth.data, 'p'):
            ...
            print('[{:.6f}] idx_packet={}, do not have eth.data.p, skip'.format(ts, idx_packet))
            continue
          ...
          # save select OCULii_timestamps
          OCULiiRadar_timestamps_str = ['{:.6f}\n'.format(t) for t in self.OCULiiRadar_timestamps]
          with open(self.OCULiiRadar_timestamps_path, 'w') as f:
            f.writelines(OCULiiRadar_timestamps_str)
          log_GREEN('generate {} done'.format(self.OCULiiRadar_timestamps_path))
    ...
    def select_generate_frame(self, timestamps, day):
      handshake_pkg = self.packets_in_frame[0]

      timestamp = handshake_pkg.timestamp_trigger
      if np.abs(timestamp - timestamps).min() > 0.1 / 2:
        timestamp_idx_matched = None
        log_YELLOW('    Skip generate {:.6f}.{}'.format(timestamp, self.pcd_file_type))
      else:
        timestamp_idx_matched = np.abs(timestamp - timestamps).argmin()
        ...
        # name the file according to the pts' unix_timestamp at 0 degree
        pcd_filename = '{}_{}.{}'.format(day, timestamp_idx_matched + 1, self.pcd_file_type)
        pcd_path = os.path.join(self.output_path, pcd_filename)
        ...
        if not os.path.exists(pcd_path):
          ...
          timestamp_idx_matched = None
          log_RED('    ({})Generate {},not save to {}'.format(len(self.frame_infos), pcd_filename, pcd_path))

      self.last_generated_frame_timestamp = handshake_pkg.timestamp_trigger

      return timestamp_idx_matched, timestamp
