...
addpath('./3rdpart/cascade_json_parser/');
addpath('./3rdpart/dataParse/');
addpath('./3rdpart/disp/');
...
% save .pcd
SAVE_PCD_ON = 1;
FRAME_IDX_GENERATE_PCD = 2;

% save reflector_info
SAVE_REFLECTOR_INFO = 1;
...
%% 
% TIRadar data sort
data_root_folder = 'F:\sensors_calibration_v2\rawdata\20221217_calibration\cameras_radars_calibration\TIRadar\20221217_162036_mode1_calib';
tmp = regexp(data_root_folder, '[0-9]{8}', 'match');
day = tmp{1};

data_infos = struct();
cnt_data_infos = 0;
data_folders = dir(data_root_folder);
for i = 1:length(data_folders)
    if data_folders(i).isdir ~= 1 || strcmp(data_folders(i).name, '.') == 1 || strcmp(data_folders(i).name, '..') == 1
        continue;
    end
    cnt_data_infos = cnt_data_infos + 1;
    data_infos(cnt_data_infos).idx = i;
    data_infos(cnt_data_infos).path = fullfile(data_folders(i).folder, data_folders(i).name);
end

%生成雷达参数文件的编号
testID = 1;

if SAVE_REFLECTOR_INFO == 1
    reflector_info = [];
end

for i_data_info = 1:length(data_infos)
    % 雷达adc数据 路径    
    dataFolder_test = data_infos(i_data_info).path;   
    % 校准矩阵 路径
    dataFolder_calib = 'mode1_256x128';    
    % 雷达设置参数模板 路径
    module_param_file = 'module_param.m';    
    % 根据原始adc数据文件夹中的config.mmwave.json文件生成matlab格式雷达参数文件 路径
    pathGenParaFile = fullfile(runs_folder, strcat('test', num2str(i_data_info), '_param.m'));
    ...
    cnt_frameGlobal = 0;
    % 查询PC端采集开始时间戳
    startTimefile = dir(fullfile(dataFolder_test, '*.triggertime.txt'));
...
%             sig_integrate = sig_integrate - base.sig_integrate;
            ...
            if PLOT_ON==1
                ...
                %横坐标range 纵坐标反射强度
                rangeList = ((1: size(sig_integrate,1))-1) * detectionObj.rangeBinSize;
                ...
                for ii=1:size(sig_integrate,2)%DopplerFFTOut的第ii列，速度不为0的列
                    ...
                    if ~isempty(detection_results)%如果通过CFAR算法检测到了目标
                        ...
                        if (~isempty(ind))%如果有监测点
                            rangeInd = detect_all_points(ind,1)-1;%取检测到的点的range
                            plot(rangeInd*detectionObj.rangeBinSize, sig_integrate(rangeInd+1,ii),'o','LineWidth',2,...
                                'MarkerEdgeColor','k',...
            ...
            if ~isempty(detection_results)%如果检测到点
                ...
                if length(angleEst) > 0%如果检测到角度结果
                    for iobj = 1:length(angleEst)%对第iobj个检测结果
                        ...
                        xyz(iobj,6) = angleEst(iobj).intensity;
                        xyz(iobj,7) = angleEst(iobj).noise_var;
                        xyz(iobj,8) = angleEst(iobj).estSNR;%取估计信噪比
                    end
                    xyz_all{cnt_frameGlobal}  = xyz;
                    
                    % 打印点云信息
                    if PRINT_INFO_ON==1
                        fprintf('%15s%15s%15s%15s%15s%15s%15s%15s\n', 'X', 'Y', 'Z', 'Velocity', 'Range', 'Intensity(dB)', 'Noise(dB)', 'estSNR(db)');
                        for iprint = 1:size(xyz,1)
                            fprintf('%15f%15f%15f%15f%15f%15f%15f%15f\n', xyz(iprint,1), xyz(iprint,2), xyz(iprint,3), xyz(iprint,4), xyz(iprint,5), xyz(iprint,6), xyz(iprint,7), xyz(iprint,8));
                        end
                    end
            ...
            if SAVE_PCD_ON && FRAME_IDX_GENERATE_PCD == frameIdx
                % save pcd
                pcd_path = fullfile(output_folder, sprintf('%s_%d.pcd', day, i_data_info));
                save_TIRadar_pcd(pcd_path, xyz);
                fprintf('save %s\n', pcd_path);
                break;
            end
...
% remove runs_folder
if exist(runs_folder, 'dir')
    [status, message, messageid] = rmdir(runs_folder, 's');
    fprintf('remove %s\n', runs_folder);
end