...
if (obj.detectMethod == 1) %进行OS CFAR检测
    ...
    if (N_obj_Rag>0) %如果距离维检测到目标
        ...
        for i_obj = 1:N_obj
            ...
            detection_results(i_obj).noise_var = 10*log10(noise_obj_agg(i_obj));       %noise variance
            ...
            detection_results(i_obj).estSNR  = 10*log10(sum(abs(detection_results (i_obj).bin_val).^2)/sum(noise_obj_agg(i_obj)));
            detection_results(i_obj).intensity = 10*log10(sum(abs(detection_results (i_obj).bin_val).^2));
